...
class Maze:
    ...
    # def make_grid(l : int,
    #              w : int) -> list:
    #     """Use the reccursive backtracking algorithm to create a maze grid
    ...
    #     description = "Creating the maze..."
    #     total_iterations = (l*w-1)  # Total number of iterations you expect
    ...
    #         #Set the cursor to the last visited cell
    #         cursor = random.choice(tuple(visited))  #By choising a random cell it gives a much better performance for large mazes sonce it does not backpropagate for a too long time BUT at the might struggle to find the few empty cells left
    #         #cursor = tuple(visited)[len(visited)-1-idx_curr]
            
    #         #If at least a cell nearby hasn't been visited yet, we start a random walk
    #         possible_path   = non_visited_neigh(cursor[0], cursor[1])
    #         if len(possible_path):
    #             visited.add(cursor)
    ...
    #         else:
    #             #Set the cursor to the last-1 visited cell
    #             idx_curr += 1
